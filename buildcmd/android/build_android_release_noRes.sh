...
$UNITY_PATH -projectPath $PROJ_PATH -executeMethod PlayerGenerator.ExportGoogleProjectNoResRelease -quit -batchmode -logFile ${PROJ_PATH}/BuildLog/build_android.log || exit
...
cp tgsw* $RELEASE_DIR/"ws_juhe_""$(date +%Y.%m.%d)"".apk"
cd $JH_DIR
python ./scripts/pack.py -g 14 -c oppo -r

cp ${JH_DIR}/output/tg2/oppo/tg2_oppo* $RELEASE_DIR/"ws_oppo_""$(date +%Y.%m.%d)"".apk"
