...
[AddComponentMenu("Script/UI/UIItem/Custom Wrap Content")]
public class UICustomWrapContent : MonoBehaviour
{
    ...
    {
        //OutLog.Log(127, "OnMove #1");
        if (mScroll.disableDragIfFits) return;//不允许拖动
        //OutLog.Log(127, "OnMove #2");
        WrapContent();
    }
	...
	public virtual void WrapContent ()
	{
		...
		else
		{
			...
			for (int i = 0, imax = mChildren.Count; i < imax; ++i)
			{
                ...
                if (distance - (-extents) < -0.01f )
                {
					...
					if ((!mScroll.isLeanTop && minIndex == maxIndex) || (minIndex <= realIndex && realIndex <= maxIndex))
					{
						t.localPosition = pos;
						UpdateItem(t, i);
					}
                ...
                else if (distance - extents > 0.01f)//else if (distance > extents)
                {
					Vector3 pos = t.localPosition;
					pos.y -= ext2;
					distance = pos.y - center.y;
        ...
        if (!mScroll.isLeanTop)
        {
            mScroll.restrictWithinPanel = !allWithinRange;
        }
        //OutLog.Log(125, "WrapContent mScroll.restrictWithinPanel=" + !allWithinRange);
