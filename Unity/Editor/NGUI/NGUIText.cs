...
static public class NGUIText
{
		...
		ref int sub, ref bool bold, ref bool italic, ref bool underline, ref bool strike, ref bool ignoreColor)
	{
		...
		if (text[index + 1] == 'u' && text[index + 2] == 'r' && text[index + 3] == 'l' && text[index + 4] == '=')
		{
            ...
            if (closingBracket != -1)
            {
                string tempStr = text.Substring(index + 4, closingBracket - index - 4);
                int specialSymbolCount = tempStr.Split('[').Length;
                for (; specialSymbolCount > 1; specialSymbolCount--)
                {
			...
			if (x > ew)
			{
				...
				if (lastLine)
				{
					...
					if (forceChLine)
					{
						start = offset;
						x = glyphWidth;
						for (int index= spaceIndex + 1; index < sb.Length; index++) {
							var tmpCH = sb[index];
							float w = GetGlyphWidth(tmpCH, prev, scale);
                            x += finalSpacingX + w;
                        }
                    }
					else
		...
		for (int i = 0; i < textLength; ++i)
		{
			...
			else // No symbol present
			{
				...
				if (underline || strikethrough)
				{
					...
					{
                        verts.Add(new Vector3(prevX, v0y - 4));
						verts.Add(new Vector3(prevX, v1y - 2));
						verts.Add(new Vector3(x, v1y - 2));
						verts.Add(new Vector3(x, v0y - 4));
					}
