...
public class ExtractUIChinese : EditorWindow
{
    [MenuItem("Tools/GetChineseTool/UIChinese", false, 10)]
    static public void OpenCameraWizard()
    {
        EditorWindow.GetWindow<ExtractUIChinese>(false, "UI文本", true).Show();
    }
    private string[] prePath = new string[] { @"Assets/BundleData/UI" };
    private const string saveStoryDlg = @"Assets/Editor/GetChineseTools/prefabUIChinese.txt";
    private const string saveStoryOnlyDlg = @"Assets/Editor/GetChineseTools/prefabUIChineseOnly.txt";
                ...
                foreach (var guid in guids)
                {
                    ...
                    foreach (var lbl in LabelList)
                    {
                        ...
                        if (!string.IsNullOrEmpty(lbl.text) && !storyDialogLst.Contains(lbl.text) && NumReg.IsMatch(lbl.text))
                        {
                            ...
                            var lblContent = DeleSpace(lbl.text);
        ...
        if (GUILayout.Button("翻译"))
        {
            ...
            foreach (var info in movLabLst)
            {
                ...
                if (null != objTr)
                {
                    ...
                    {
                        lbl.text = RevertSpace(fanyiInfo.fanyi);
                        IsFanYi = true;
                    }
                    else
                    {
    ...
    /// <returns></returns>
    static string DeleSpace(string src) {
        return src.TrimEnd().Replace("\n\r", "#NODEL#").Replace("\n", "#NODEL2#").Replace("\r", "#NODEL3#");
    }
    static string RevertSpace(string src)
    {
        return src.Replace("#NODEL#", "\n\r").Replace("#NODEL2#", "\n").Replace("#NODEL3#", "\r");
    }
}
