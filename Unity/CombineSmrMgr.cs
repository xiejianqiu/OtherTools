...
public class CombineSmrMgr : ComponentBase
{
    ...
    public override void OnDestroy()
    {
        ...
        if (null != m_FacePart)
        {
            GameObject.Destroy(m_FacePart);
        }
    }
    #endregion
    bool CheckAndUseDefaultFace()
    {
        if (UseDefaultFace)
        {
            LoadPart(GameObject.Instantiate(m_FaceModels[0]), m_FaceObj, m_BodyBoneObj, (int)KneadFaceDataType.Face);
            return true;
        }
        return false;
    }
    ...
    private void ChangeSmr(int modelId, KneadFaceDataType param)
    {
        ...
        //{
        //    LogModule.ErrorLog("Tab_KneadfaceOClothing 出錯");
        //    return;
        //}
    ...
    private void LoadPart(GameObject newPartInst, object param1, object param2, object param3)
    {
        ...
        if (partObj == null)
        {
            LogModule.ErrorLog("This part is null");
            return;
        }

        if (boneObj == null)
        {
            LogModule.ErrorLog("This boneObj is null");
            return;
        }

        SkinnedMeshRenderer smr = newPartInst.GetComponentInChildren<SkinnedMeshRenderer>();
        ...
        switch (partType)
        {
            ...
            case (int)KneadFaceDataType.Hair:
                ...
                m_HairPart = newPartInst;
                GameObject.DontDestroyOnLoad(m_HairPart);
                break;
            case (int)KneadFaceDataType.Face:
                if (m_FacePart != null)
                {
                    GameObject.Destroy(m_FacePart);
                }
                m_FacePart = newPartInst;
                GameObject.DontDestroyOnLoad(m_FacePart);
                break;
        }
        //Destroy(obj);
    }
