...
public class PatchBuilder
{
    ...
    [MenuItem("ABSystem/GenUpdateInfo")]
    public static void GenUpdateInfo() {
        var dirPath = EditorUtility.OpenFolderPanel("补丁", "","");
        var files = Directory.GetFiles(dirPath, "*", SearchOption.AllDirectories);
        string PatchInfoFile = dirPath + PathConfig.m_UpdateInfoFileName;
        if (File.Exists(PatchInfoFile)) {
            File.Delete(PatchInfoFile);
        }
        using (FileStream fs = new FileStream(PatchInfoFile, FileMode.Append, FileAccess.Write))
        {
            StreamWriter sw = new StreamWriter(fs);
            foreach (var file in files)
            {
                if (file.EndsWith(".ab")|| file.EndsWith(".all"))
                {
                    FileInfo fi = new FileInfo(file);

                    JsonData jdDll = new JsonData();
                    jdDll["fn"] = fi.Name;
                    jdDll["md"] = GfxUtils.GetMD5Hash(file);
                    jdDll["size"] = fi.Length;
                    sw.WriteLine(jdDll.ToJson());
                }
            }
