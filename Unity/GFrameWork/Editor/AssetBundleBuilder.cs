...
namespace GameFramework
{
    public class AssetBundleBuilder : ABBuilder
    {
        ...
        public override List<AssetTarget> Export(bool buildAssetbundle=true)
        {
            ...
            if (buildAssetbundle)
            {
                BuildAssetBundleOptions opts = BuildAssetBundleOptions.ChunkBasedCompression | BuildAssetBundleOptions.DisableWriteTypeTree;
                if (EnvUtils.IsUNITY_WEBGL())
                {
                    opts = BuildAssetBundleOptions.None | BuildAssetBundleOptions.DisableWriteTypeTree;
                }
                //开始打包
