    ...
    public sealed partial class AutoDownloadRes
    {
        ...
        public void DownResCfg(string cfgUrl)
        {
            ...
            Action<string, bool> OnCallBack = (a, b) =>
            {
                if (!b)
                {
                    StartCoroutineManager.inst.StartCoroutine(this.InitCfg(savePath));
                }
                else
                {
                    DownResCfg(cfgUrl);
        ...
        private IEnumerator InitCfg(string filePath)
        {
            var cacheDir = AssetBundleManager.Instance.pathResolver.BundleCacheDir;
            ...
            {
                yield return CheckAndSleep();
                var line = lines[tindex];
                var bundleName = line.TrimEnd('\n').TrimEnd('\r');
                if (infoMap.TryGetValue(bundleName, out var info))
                {
                    if (!EnvUtils.IsUNITY_WEBGL() && AssetBundleManager.Instance.IsPackAsset(info.fullNameWithHash))
                    {
                        continue;
                    }
                    builder.Clear();
        ...
        public IEnumerator CleanDirtyResCo()
        {
            var cacheDir = AssetBundleManager.Instance.pathResolver.BundleCacheDir;
            var infoMap = AssetBundleManager.Instance.depInfoReader.infoMap;
            #endregion
            ...
            foreach (var file in tmpFiles)
            {
                ...
                if ((curTime - fileInfo.LastWriteTime).TotalMinutes > 10)
                {
        ...
        {
            if (AssetBundleManager.Instance.isCurrentLoading)
                return;
            Queue<AssetBundleData> queue = null != preloadQueue && preloadQueue.Count > 0 ? preloadQueue : waitForDLQueue;
            ...
            this.MAX_REQ = null != preloadQueue && preloadQueue.Count > 0 ? 30 : ClientCfg.MyClientCfg.nofaudodl;

            if (CntPerSecond > 0 && (this.GetNumOfIdleReq() > 0 && (CanDLNextResByCo() || CanDLNextResByLoader())))
            {
                CntPerSecond -= 1;
            ...
            if (CntPerSecond <= 0 && Time.time > nextBeginTime)
            {
                CntPerSecond = CNT_PER_SECOND;
                nextBeginTime = Time.time + 1;
            }
        }
