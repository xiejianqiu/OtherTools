...
namespace GameFramework
{
    ...
    public partial class AssetTarget : System.IComparable<AssetTarget>
    {
        ...
        public bool IsReAnalyzed = false;

        public bool IsPrefabChild = false;
        ...
        public List<string> assetsPathLst = new List<string>();
        /// <summary>
        /// 是否是场景相关需要单独打包的文件(unity,fbx,mat)
        /// </summary>
        public bool IsSceneStandalone = false;
        public AssetTarget(Object o, FileInfo file, string assetPath)
        {
            ...
            _isFileChanged = true;
            _metaHash = "0";
            if (fullName.EndsWith(".unity"))
            {
                IsSceneStandalone = true;
            }
        }
        public AssetTarget(Object o, DirectoryInfo dirInfo, string assetPath)
        {
            this.asset = o;
            this.fullName = dirInfo.FullName+"Pack.txt";
            this.assetPath = assetPath;
            this.bundleShortName = (dirInfo.Name+ "Pack.txt").ToLower();
            this.bundleName = HashUtil.Get(AssetBundleUtils.ConvertToABName(assetPath)) + ".ab";
            this.bundleSavePath = Path.Combine(AssetBundleUtils.pathResolver.BundleSavePath, bundleName);
        ...
        /// <summary>
        /// 当发现某个prefab是其它asset的依赖项的时候，需要把该prefab的一级依赖标记为standalone
        /// </summary>
        public void ReAnalyze()
        {
            ...
            for (int i = 0; i < deps.Length; i++)
            {
                ...
                {
                    //Debug.LogError("Contains Resources ---->" + path);
                    //    continue;
                }
                depList.Add(o);
            }
            ...
            for (int i = 0; i < paths.Length; i++)
            {
                ...
                if (target == null)
                    continue;
                target.exportType = AssetBundleExportType.Standalone;
            }
        }
        /// <summary>
        /// 分析引用关系
        ...
        public void Analyze()
        {
            ...
            for (int i = 0; i < paths.Length; i++)
            {
                ...
                if (this.IsSceneStandalone)
                {
                    if (target.fullName != null)
                    {
                        if (target.fullName.ToLower().EndsWith(".fbx") || target.fullName.ToLower().EndsWith(".mat"))
                        {
                            target.IsSceneStandalone = true;
                        }
                    }
                }
                else
                {
                    if (target.fullName != fullName && target.fullName.EndsWith(".prefab"))       //当依赖的是一个预制体，就将这个预制体标记为是一个被依赖的子预制体
                    {
                        target.IsPrefabChild = true;
                    }
                    if (IsPrefabChild)          //如果this是一个其他资源所依赖的预制体，this所有依赖的东西都设置为单独打包,否则this所依赖的所有资源都会在依赖this的资源中冗余
                    {
                        target.exportType = AssetBundleExportType.Standalone;
                    }
                }
        ...
        public void AddPackAsset(FileInfo vFileInfo)
        {
            string fullPath = vFileInfo.FullName;
            int index = fullPath.IndexOf("Assets");
            if (index != -1)
            {
                string assetPath = fullPath.Substring(index);
                assetsPathLst.Add(assetPath);
            }
        }
        ...
        public void BeforeExport()
        {
            ...
            if (!CheckCanStandalone())
                return;
            if (this.exportType == AssetBundleExportType.Asset)
            {
                if (this.assetPath.EndsWith("LightingData.asset"))
                    return;
        ...
        public bool needSelfExport
        {
            get
            {
                ...
                bool v = exportType == AssetBundleExportType.Root || exportType == AssetBundleExportType.Standalone||IsSceneStandalone;
        ...
        public string GetHash()
        {
            if (type == AssetType.Builtin)
                ...
                return AssetBundleUtils.GetFileHash(fullName);
        }
